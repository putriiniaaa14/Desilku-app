...
export async function saveDataUpdateSubmission(userId, { formData, files, indikasi }) {
  ...
  const payload = {
    ...formData,
    jenis_pengajuan: formData?.jenisPengajuan || "Pemutakhiran Data Sosial Ekonomi",
    status_pengajuan: "Menunggu Verifikasi",
    indikasi_desil_aplikasi: indikasi ?? null,
  };
...
export async function getMySubmissions() {
  const { data, error } = await supabase
    .from("desil_submissions")
    .select("id, created_at, data, indikasi, status")
  ...
  return (data || []).map((row) => {
    ...
    return {
      ...row,
      status: row.status || row.data?.status_pengajuan || "Menunggu Pemeriksaan",
      nomorPengajuan: `DSK-${created.getFullYear()}${String(created.getMonth() + 1).padStart(2, "0")}${String(created.getDate()).padStart(2, "0")}-${String(row.id).slice(0, 4).toUpperCase()}`,
    };
  });
}
