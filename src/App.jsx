...
function AdminScreen({ onBack }) {
  ...
  return <div className="min-h-screen bg-[#f7faf9]">
    <Header title="Admin · Verifikasi" onBack={onBack}/>
    <main className="mx-auto max-w-6xl px-4 py-7">
      <div className="mb-6 rounded-3xl border border-blue-100 bg-gradient-to-br from-blue-50 via-white to-emerald-50 p-6 shadow-sm">
        <div className="flex flex-wrap items-start justify-between gap-4">
          <div><p className="text-sm font-bold text-blue-600">Dashboard Admin</p><h1 className="mt-1 text-2xl font-black text-slate-900">Verifikasi Pengajuan Warga</h1><p className="mt-2 max-w-2xl text-sm leading-6 text-slate-600">Periksa data dan lampiran, lalu ubah status pengajuan. Verifikasi ini hanya pemeriksaan pengajuan aplikasi, bukan penetapan desil resmi.</p></div>
          <button onClick={load} className={`${press} rounded-xl border border-slate-200 bg-white px-4 py-2.5 text-sm font-bold text-slate-700 hover:bg-slate-50`}>↻ Muat ulang</button>
        </div>
      </div>
      ...
      <div className="grid gap-5 lg:grid-cols-[1.05fr_.95fr]">
        <section className="rounded-3xl border border-slate-100 bg-white p-5 shadow-sm">
          <div className="mb-4 flex items-center justify-between"><div><h2 className="font-black text-slate-900">Daftar Pengajuan</h2><p className="text-xs text-slate-500">{items.length} pengajuan ditemukan</p></div></div>
          {loading ? <div className="py-12 text-center text-sm text-slate-500">Memuat pengajuan...</div> : items.length === 0 ? <div className="py-12 text-center text-sm text-slate-500">Belum ada pengajuan.</div> : <div className="space-y-3">{items.map(item => <button key={item.id} onClick={()=>{setSelected(item);setNotice("")}} className={`${press} w-full rounded-2xl border ${selected?.id===item.id?"border-blue-300 bg-blue-50/50":"border-slate-100 bg-white hover:border-blue-200 hover:bg-slate-50"} p-4 text-left`}>
            <div className="flex items-start justify-between gap-3"><div className="min-w-0"><p className="font-black text-slate-900 truncate">{getName(item)}</p><p className="mt-1 text-xs text-slate-500">{item.nomor_pengajuan || item.id?.slice(0,8)} · {getDate(item)}</p></div><StatusPill status={getStatus(item)}/></div>
          </button>)}</div>}
        </section>
        <section className="rounded-3xl border border-slate-100 bg-white p-5 shadow-sm">
